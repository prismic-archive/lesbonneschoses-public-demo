...
class SynchronizedLruMap[K, V](maxSize: Int, underlying: util.Map[K, V]) extends LruMap[K, V](maxSize, util.Collections.synchronizedMap(underlying)) with SynchronizedMap[K, V]{
  def this(maxSize: Int) = this(maxSize, LruMap.makeUnderlying(maxSize))
}